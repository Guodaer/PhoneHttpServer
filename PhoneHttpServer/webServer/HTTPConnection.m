	...
	if (!allowDirectory)
	{
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:fullPath isDirectory:&isDir] && isDir)
		{
			...
			for (NSString *indexFileName in indexFileNames)
			{
				...
				{
                    XC_DebugLog(@"🍎🍎🍎-- in %@",indexFilePath);//在这里把index.html的目录改成沙盒的
...
{
//    NSString *str111 = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
//    NSLog(@"httpConnection2015🍎🍎🍎 -- %@",str111);
...
//            NSLog(@"httpConnection2062============= %@",contentLength);
